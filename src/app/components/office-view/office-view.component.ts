...
export class OfficeViewComponent {
  ...
  constructor(    private officeService: OfficeService,    private readonly _cdRef: ChangeDetectorRef,){ }
  ...
  fetchOffices() {
    ...
    this.officeService.getOffices().pipe(takeUntil(this._destroy$)).subscribe((officeList) => {
      if(officeList) {this.officeEntries = officeList.map(o => ({id: o.id, name: o.location}))};
  ...
  selectEntry(id:number): void {
    this.officeService.getOffice(id).pipe(takeUntil(this._destroy$)).subscribe((off) => {
      if(off) {this.officeResponse = off};
  ...
  createOffice(): void{
    ...
    this.officeService.postOffice({location: 'Nowe Miasto', id:0}).pipe(takeUntil(this._destroy$)).subscribe((off) => {
      if(off) {this.officeResponse = off};
  ...
  modifyOffice(office: Office): void {
    this.officeService.putOffice(office).pipe(takeUntil(this._destroy$)).subscribe((off) => {
      if(off) {this.officeResponse = off};
      this.fetchOffices();
    });
  }
